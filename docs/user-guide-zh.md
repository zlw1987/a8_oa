...
如果缺少必需附件，系统会创建 MISSING_RECEIPT 会计复核事项，并阻止申请关闭。申请级附件仍用于报价单、合同和一般支持文件。收据和发票现在可以从 PR/TR 详情页关联到具体 actual expense line。Missing receipt review 会指向具体缺少支持文件的 actual expense line。
