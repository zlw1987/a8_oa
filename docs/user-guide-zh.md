...
Dashboard 会根据角色显示不同卡片：

- 申请人：草稿、退回给我、待审批、已批准未关闭、缺少收据 / 需要我处理、创建 PR/TR。
- 审批人：我的待审批任务、逾期审批任务、可领取审批池任务、最近审批、退回 / 拒绝事项。
- 会计：待处理会计复核、缺少收据、超预算复核、需要补充申请、未匹配公司卡、重复公司卡复核。
- 财务管理员：复核 aging、接近预算上限的项目、超预算例外、未匹配公司卡数量、收据政策问题、财务报表和政策设置快捷入口。
- 系统管理员：用户 / 部门设置、审批规则设置、财务政策设置和系统提示。
